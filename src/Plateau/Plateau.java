...
public class Plateau {
    ...
    public void repartirRessourceGui(int resultatDe) {
       // System.out.println("Le résultat du dé est " + resultatDe);
        for (int i = 0; i < plateau.length; i++) {
            for (int j = 0; j < plateau[i].length; j++) {
                if (plateau[i][j].getNumero() == resultatDe) {
    ...
    public int getNbChevalierMax(){
        return nbChevalierMax;
    }
    public CarteChevalierLePlusPuissant getChevalierLePlusPuissant(){
        return chevalierPuissant;
    }

    public void setChevalierMax(int nbChevalierMax){
        this.nbChevalierMax = nbChevalierMax;
    }
}
